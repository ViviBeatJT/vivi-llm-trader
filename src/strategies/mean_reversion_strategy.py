...
def backtest_arbitrary_period(ticker: str, start_dt: datetime, end_dt: datetime, step_minutes: int = 5):
    """
    ...
    # 打印回测范围
    print(f"\n--- 🚀 开始回测 ({ticker}) ---")
    print(f"回测范围: {start_dt.strftime('%Y-%m-%d %H:%M UTC')} 至 {end_dt.strftime('%Y-%m-%d %H:%M UTC')}")
    print("-" * 50)
    ...
    while current_time <= end_dt:
        ...
        signal_result = get_mean_reversion_signal(ticker=ticker, end_dt=current_time)
    ...
    if action_signals:
        ...
        for r in action_signals:
            print(f"  {r['timestamp_utc'].strftime('%Y-%m-%d %H:%M UTC')} | {r['signal']:4} | 置信度: {r['confidence']}/10 | 原因: {r['reason']}")
    else:
...
if __name__ == '__main__':    
    ...
    # 回测起始时间
    START_DATE = datetime(2025, 12, 4, 20, 0, 0, tzinfo=timezone.utc) 
    
    # 回测结束时间
    END_DATE = datetime(2025, 12, 4, 21, 0, 0, tzinfo=timezone.utc) # 仅测试 12 月 4 日收盘前
