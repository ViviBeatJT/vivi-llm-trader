...
class LiveEngine:
    ...
    def _log_status(self, current_price: float):
        ...
        if account_status['position'] != 0:
            ...
            if account_status['position'] > 0:
                unrealized_pnl = (
                    current_price - account_status['avg_cost']) * account_status['position']
            else:
                unrealized_pnl = (
                    account_status['avg_cost'] - current_price) * abs(account_status['position'])
            ...
            if self.on_signal_callback:
                try:
                    self.on_signal_callback(
                        force_close_signal, current_price, now_utc)
    ...
    def _run_single_iteration(self) -> bool:
        ...
        try:
            # ✨ 传递收盘时间信息给策略
            is_close_to_market_close = is_force_close_time(
                now_et, self.force_close_time) if self.force_close_time else False

            signal_data, updated_df = self.strategy.get_signal(  # ✅ 改名为 updated_df
                ticker=self.ticker,
                ...
                # initial_capital=initial_cash,  # ✅ 添加这两个参数
                # current_equity=account_status['equity']             # ✅ 用于账户止损
                )
            ...
            # ✅ 从 signal_data 中获取价格，如果没有就用 current_price
            strategy_price = signal_data.get('price', current_price)
            if strategy_price > 0:
        ...
        if signal in ["BUY", "SELL", "SHORT", "COVER"]:
            ...
            signal_emoji = {"BUY": "🟢", "SELL": "🔴",
                "SHORT": "🔻", "COVER": "🔺"}.get(signal, "⚪")
            print(f"\n{signal_emoji} [{format_time_et(now_et)}] 交易信号!")
            print(
                f"   信号: {signal} | 价格: ${current_price:.2f} | 置信度: {confidence}/10")
            print(f"   原因: {reason}")
    ...
    def run(self) -> dict:
        ...
        # 检查初始账户状态
        initial_status = self.position_manager.get_account_status(
            current_price=0.0)
        print(f"\n💰 初始账户状态:")
        ...
        try:
            while self._running:
                ...
                if self.respect_market_hours and not self._is_market_open():
                    ...
                    print(
                        f"   距离开盘: {self._format_duration(time_until_open.total_seconds())}")
    ...
    def _generate_report(self) -> dict:
        ...
        # ✨ 改进
        print(f"   最终持仓: {final_status['position']:.0f} 股 {'✅' if final_status['position'] == 0 else '⚠️'}")
        print("="*60)
