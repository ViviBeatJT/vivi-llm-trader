...
def run_single_day_backtest(
    ...
    try:
        ...
        print(f"股票: {ticker}")
        print(
            f"策略: {strategy_name} ({STRATEGY_CONFIGS[strategy_name]['name']})")
        ...
        while current_time <= end_time:
            ...
            # 获取信号
            current_et = current_time.astimezone(
    ...
    try:
        ...
        if total_pnl == 0.0 and 'equity' in final_status and 'cash' in final_status:
            ...
            total_pnl_pct = (total_pnl / initial_capital *
                             100) if initial_capital > 0 else 0.0
        ...
        if not trade_log.empty and 'type' in trade_log.columns:
            ...
            if not completed_trades.empty and 'net_pnl' in completed_trades.columns:
                winning_trades = len(
                    completed_trades[completed_trades['net_pnl'] > 0])
                losing_trades = len(
                    completed_trades[completed_trades['net_pnl'] < 0])
                win_rate = winning_trades / \
                    len(completed_trades) if len(completed_trades) > 0 else 0
            else:
                winning_trades = 0
                losing_trades = 0
                win_rate = 0
...
def run_bulk_backtest(
    ...
    if consecutive_capital and not df.empty:
        for strategy in df['strategy'].unique():
            ...
            df.loc[strategy_mask,
                   'cumulative_equity'] = df.loc[strategy_mask, 'final_equity']
...
def generate_summary_reports(df: pd.DataFrame, output_dir: str):
    ...
    quarterly_summary['year_quarter'] = quarterly_summary['year_quarter'].astype(
        str)
    quarterly_summary.to_csv(
        f"{output_dir}/quarterly_summary.csv", index=False)
    ...
    strategy_comparison.columns = ['_'.join(col).strip(
        '_') for col in strategy_comparison.columns.values]
    strategy_comparison.to_csv(
        f"{output_dir}/strategy_comparison.csv", index=False)
    print(f"✅ 策略对比: {output_dir}/strategy_comparison.csv")

    # === 6. 生成文本报告 ===
    generate_text_report(df, yearly_summary, monthly_summary,
                         strategy_comparison, output_dir)
...
def generate_text_report(df: pd.DataFrame, yearly: pd.DataFrame, monthly: pd.DataFrame,
    ...
    for strategy in df['strategy'].unique():
        ...
        report_lines.append(f"\n{strategy}:")
        report_lines.append(
            f"   最佳: {best_day['date'].strftime('%Y-%m-%d')} - ${best_day['pnl']:+.2f} ({best_day['pnl_pct']:+.2f}%)")
        report_lines.append(
            f"   最差: {worst_day['date'].strftime('%Y-%m-%d')} - ${worst_day['pnl']:+.2f} ({worst_day['pnl_pct']:+.2f}%)")
